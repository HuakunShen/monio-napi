...
use monio::{
    display_at_point, displays, key_press, key_release, key_tap, mouse_click,
    mouse_move, mouse_press, mouse_release, primary_display, system_settings, Button,
...
impl HookJs {
    ...
    pub fn stop(&self) -> Result<()> {
        ...
        if let Some(hook) = guard.take() {
            hook.stop()
                .map_err(|e| Error::new(Status::GenericFailure, format!("Failed to stop hook: {}", e)))?;
        }
        Ok(())
    }
...
pub fn get_displays() -> Result<Vec<DisplayInfoJs>> {
    displays()
        ...
        .map_err(|e| Error::new(Status::GenericFailure, format!("Failed to get displays: {}", e)))
}
...
pub fn get_primary_display() -> Result<DisplayInfoJs> {
    primary_display()
        .map(|info| (&info).into())
        .map_err(|e| Error::new(Status::GenericFailure, format!("Failed to get primary display: {}", e)))
}
...
pub fn get_display_at_point(x: f64, y: f64) -> Result<Option<DisplayInfoJs>> {
    display_at_point(x, y)
        ...
        .map_err(|e| Error::new(Status::GenericFailure, format!("Failed to get display at point: {}", e)))
}
...
pub fn get_system_settings() -> Result<SystemSettingsJs> {
    system_settings()
        ...
        .map_err(|e| Error::new(Status::GenericFailure, format!("Failed to get system settings: {}", e)))
}
...
pub fn simulate_mouse_press(button: ButtonJs) -> Result<()> {
    mouse_press(button.into())
        .map_err(|e| Error::new(Status::GenericFailure, format!("Failed to press mouse button: {}", e)))
}

/// Release a mouse button
#[napi]
pub fn simulate_mouse_release(button: ButtonJs) -> Result<()> {
    mouse_release(button.into())
        .map_err(|e| Error::new(Status::GenericFailure, format!("Failed to release mouse button: {}", e)))
}

/// Click a mouse button (press + release)
#[napi]
pub fn simulate_mouse_click(button: ButtonJs) -> Result<()> {
    mouse_click(button.into())
        .map_err(|e| Error::new(Status::GenericFailure, format!("Failed to click mouse button: {}", e)))
}
...
pub fn simulate_key_release(key: KeyJs) -> Result<()> {
    key_release(key.into())
        .map_err(|e| Error::new(Status::GenericFailure, format!("Failed to release key: {}", e)))
}
